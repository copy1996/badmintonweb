...
Page({
  ...
  onShow: function () {
    console.log("activity.js onshow begin to work")
    this.setData({
      matchList: [],
      bottomNum: 1,
      hasToEnd: false
  ...
  gotoDetails(e){
    ...
    const beginTime = e.currentTarget.dataset.begintime;
    const endTime = e.currentTarget.dataset.endtime;
    const address = e.currentTarget.dataset.address;
    const participantNum = e.currentTarget.dataset.participantnum;
    const limitNum = e.currentTarget.dataset.limitnum;
    const ownerName = e.currentTarget.dataset.ownername;
    const phone = e.currentTarget.dataset.phone;
    const localtime = e.currentTarget.dataset.localtime;
    console.log("localtime:", localtime)
    // localtime: '',
    console.log("matchid:", matchId)
    wx.navigateTo({
      url: '../enroll/enroll?matchid=' + matchId + '&deadline=' + deadline + '&gameType=' + gameType + '&beginTime=' + beginTime + '&endTime=' + endTime + '&address=' + address + '&participantNum=' + participantNum + '&limitNum=' + limitNum + '&ownerName=' + ownerName + '&phone=' + phone + '&localtime=' + localtime
    })
  }
})
...
function getActivities(that){
  ...
  request.oneRequest.result(param).then(res => {
    console.log("that data bottomNum:", that.data.bottomNum)
    var resJson = res.data.result.data;
    var previousMatchData = that.data.matchList;
    console.log("matchList:", that.data.matchList)
