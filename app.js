        ...
        success: function (res) {
          ...
          wx.request({
            ...
            success: function (res) {
              //存储session
              that.globalData.sessionId = res.data.session;
              console.log("app.js that.globalData.sessionId", that.globalData.sessionId)
              wx.getUserInfo({
                success: function (resInfo) {
                  console.log("app.js wx.getuserinfo:", resInfo)
                  that.globalData.userInfo = resInfo.userInfo;
                  typeof cb == "function" && cb(that.globalData.userInfo);
                  console.log("app.js: data:", resInfo.userInfo)
                  //向后台传输保存用户信息
                  wx.request({
                    url: API.APIDomian + '/wx/user/check',
                    header: {
                      'content-type': 'application/x-www-form-urlencoded;charset=utf-8',
                      'Cookie':'token='+that.globalData.sessionId
                    },
                    method: 'POST',
                    data: {
